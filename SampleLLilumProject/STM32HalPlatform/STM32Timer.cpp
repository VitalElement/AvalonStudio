...
STM32TimerInterrupt::STM32TimerInterrupt (STM32Timer* owner, uint32_t interruptNumber)
{
    this->owner = owner;
...
STM32Timer::STM32Timer (TIM_HandleTypeDef* handle, uint32_t interruptNumber, uint32_t clockFrequency, float desiredFrequency, uint32_t period)
    : interrupt (this, interruptNumber)
{
    ...
    handle->Init.Prescaler = (uint32_t) ((clockFrequency / desiredFrequency) - 1);
