...
namespace AvalonStudio.Controls.Standard.CodeEditor
{
    public class CodeEditor : AvaloniaEdit.TextEditor
    {
        ...
        public CodeEditor() : base(new TextArea(), null)
        {
            ...
            Observable.FromEventPattern(TextArea.Caret, nameof(TextArea.Caret.PositionChanged)).Throttle(TimeSpan.FromMilliseconds(100)).ObserveOn(RxApp.MainThreadScheduler).Subscribe(e =>
            {
                if(Document != null)
                {
                    var location = new TextViewPosition(Document.GetLocation(CaretOffset));
