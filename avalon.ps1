...
Write-Host "Preparing to run AvalonBuild..."
...
if (-Not $SkipToolPackageRestore.IsPresent) {
    ...
    $NuGetOutput = Invoke-Expression "&`"$NUGET_EXE`" install -PRE -ExcludeVersion -OutputDirectory `"$TOOLS_DIR`""
...
if (!(Test-Path $AVALON_EXE)) {
    Throw "Could not find AVALON.exe at $AVALON_EXE"
}

# Start Cake
Invoke-Expression "$AVALON_EXE --version"

Invoke-Expression "& `"$AVALON_EXE`" $ScriptArgs"
exit $LASTEXITCODE